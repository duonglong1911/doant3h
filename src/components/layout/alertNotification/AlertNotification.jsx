...
export default class AlertNotification extends Component {
  ...
  render() {
    ...
    return (
      <div
        className={!isCloseNotice ? `alert show ${typeColor}` : "alert hide"}
      >
        <div className={typeColor}>
          <span
            className={`fas fa-exclamation-circle alertIcon ${typeColor}`}
          ></span>
          <span className={`alertMessage ${typeColor}`}>{noticeContent} </span>
          <span
            onClick={this.handleCloseNotice}
            className={`alertCloseBtn ${typeColor}`}
          >
